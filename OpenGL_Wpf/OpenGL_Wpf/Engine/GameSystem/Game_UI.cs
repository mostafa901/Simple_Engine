...
namespace InSitU.Views.ThreeD.Engine.GameSystem
{
    public class Game_UI
    {
        ...
        private void Create_LoadCamera(Imgui_MenuItem filemenu)
        {
            ...
            {
                string path = UT_System.LoadFiles(Core.Serialize.Import.GetFilter(Core.Serialize.Import.filter.InsituModel)).FirstOrDefault();
                if (path != null)
                {
        ...
        private void Create_SaveCamera(Imgui_MenuItem filemenu)
        {
            ...
            {
                string path = UT_System.SaveFilePath(Core.Serialize.Import.GetFilter(Core.Serialize.Import.filter.InsituModel));
                if (path == null) return;
        ...
        private void Create_ImportModels(Imgui_MenuItem filemenu)
        {
            var imgui_Importbutton = new Imgui_Button(filemenu, "Import Model",
                        (x) =>
                        {
                            ...
                            if (path != null)
                            {
                                GameFactory.DrawInsituGeometry(game.ActiveScene, path);
                            }
